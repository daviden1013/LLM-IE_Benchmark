...
with open(os.path.join(PATH, 'benchmarks', 'PHI de-identification 2014', 'doc_id', 'test_id')) as f:
    lines = f.readlines()
test_ids = [line.strip() for line in lines]

count_list = []
filenames = os.listdir(os.path.join(PATH, 'benchmarks', 'PHI de-identification 2014', 'IE'))
...
for filename in loop:
    ...
    ie = Information_Extraction_Document(doc_id=doc_id, filename=os.path.join(PATH, 'benchmarks', 'PHI de-identification 2014', 'IE', filename))
